...
const door = new THREE.Mesh(
    new THREE.PlaneGeometry(doorWidth, doorHeight, 100, 100),//displacementMap
    new THREE.MeshStandardMaterial({
...
door.position.z = (wallsDepth * 0.5) + 0.01;
...
bush4.position.set(- 1.3, 0.05, 2.4);
...
for (let i = 0; i < 50; i++) {
    ...
    grave.rotation.set(0, (Math.random() - 0.5) * 0.6, (Math.random() - 0.5) * 0.6)
...
renderer.setSize(sizes.width, sizes.height)
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2))
renderer.setClearColor('#262837') //fog
